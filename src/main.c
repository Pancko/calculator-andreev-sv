...
double division(double first, double second)
{
    ...
    if (is_float) return first/second;
...
double zzz(double result)
{
    ...
    if (operation == ')')
    {
        if (current_sym >= 1 && symbols[current_sym - 1] == '(') return res;
        current_sym ++;
    ...
    if (current_sym < sym && symbols[current_sym + 1] == '(')
    {
        ...
        switch(operation)
        {
            case '+' : return res + zzz(second);
            case '-' : return res - zzz(second);
            case '*' : return res * zzz(second);
            case '/' : return division(res, zzz(second));
        }
    }
    current_sym++;
    switch (operation)
    {
        ...
        case '*': return res * second;
        case '/': return division(res, second);
    }
}
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < argc; i++) 
        if (strcmp(argv[i], "--float") == 0) is_float = 1;
        else is_float = 0;
    ...
    if (is_float == 1) printf("%.4f", result);
